...
//      var dx = determineDx(arry[i], size);
//        var windowwidth = document.querySelector("#randomPizzas").offsetWidth;
       // var newwidth = (document.querySelectorAll(".randomPizzaContainer").offsetWidth, size) + "%";
       //var newwidth;
       switch(size) {
        case "1":
          newwidth = 25;
      ...
      var randomizzas = document.querySelectorAll(".randomPizzaContainer");

      for (var i = 0; i < randomizzas.length; i++) {
        randomizzas[i].style.width = newwidth + "%";
    }}
...
// generate 5 values which always repeats in the loop  
  for (var i = 0; i < 5; i++) {
//    var phase = Math.sin((top / 1250) + (i % 5));
//After generating place them in the array.
    box1.push(Math.sin((top / 1250) + i));
  }
  //make the positions of each values changed
    for (var i = 0; i < items.length; i++) {
      var phase = box1[i % 5];

      var left = items[i].basicLeft + 100 * phase - halfScreenWidth;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var row = Math.floor(intViewportHeight / s )+1;
  var test = cols * row;
